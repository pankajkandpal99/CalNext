...
const getData = async (userName: string, selectedDate: Date) => {
  ...
  const data = await prisma.availability.findFirst({
    where: {
      day: currDay as Prisma.EnumDayFilter, // get enum of Day from schema.prisma file
      User: {
        userName: userName,
      },
    },
...
const calculateAvailableTimeSlots = (
  ...
  const allSlots = [];
  let currSlot = availableFrom;

  while (isBefore(currSlot, availableTill)) {
    allSlots.push(currSlot);
    currSlot = addMinutes(currSlot, duration);
  }
...
export const TimeTable = async ({
  ...
  // console.log("nylasCalendarData : ", nylasCalendarData);
